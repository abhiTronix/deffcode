...
if [ $OS_NAME = "linux" ]; then
  ...
  nohup sudo ffmpeg -hide_banner -loglevel error -re -stream_loop 20 -i "$TMPFOLDER"/Downloads/Test_videos/BigBuckBunny_4sec_VO.mp4 -f v4l2 -pix_fmt yuv420p /dev/video2 &
  echo "Done"
fi
